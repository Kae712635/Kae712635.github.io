...
export default function TextAlternative3D({ onSelectProject }) {
  ...
  return (
    <div className="space-y-4 text-[#F5EBDD] mt-4 border-t border-[#D4A24E]/25 pt-4">
      {/* Banner */}
      <div className="bg-[#3C6E71]/20 border border-[#3C6E71]/60 p-3.5 rounded-xl flex items-center justify-between flex-wrap gap-3">
        <div>
          <h4 className="font-cinzel text-sm font-bold text-[#D4A24E]">
            {language === 'fr' ? 'Version Texte Complète (Sans 3D)' : 'Complete Text Version (No 3D)'}
          </h4>
          <p className="text-xs text-[#D8C6B6] mt-0.5 font-sans">
            {language === 'fr' 
        ...
        <button
          ...
          className="px-3 py-1.5 bg-[#A6303B] hover:bg-[#801F29] text-white text-xs font-cinzel font-bold rounded-lg transition-colors cursor-pointer"
        >
        ...
        {[
          ...
          <button
            ...
            className={`px-3 py-2 rounded-lg text-xs font-cinzel font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
              activeBay === bay.id
                ? 'bg-[#D4A24E] text-[#1E0A0E] shadow'
                : 'bg-[#1E0A0E] text-[#D8C6B6] border border-[#D4A24E]/30 hover:text-[#F5EBDD] hover:border-[#D4A24E]/60'
            }`}
          >
      ...
      {/* Panel Content */}
      <div className="bg-[#1E0A0E] p-4 rounded-xl border border-[#D4A24E]/25 max-h-[42vh] overflow-y-auto custom-scrollbar font-sans">
          ...
          <div id="panel-bay-1" role="tabpanel" aria-labelledby="tab-bay-1" className="space-y-3">
            ...
            {cvData.experiences.map((exp) => (
              <article key={exp.id} className="p-3 bg-[#2B0F14] border border-[#D4A24E]/20 rounded-lg">
                ...
                {exp.highlights && (
                  <ul className="list-disc list-inside text-[11px] text-[#D8C6B6]/90 space-y-0.5">
        ...
        {activeBay === 'bay-2' && (
          <div id="panel-bay-2" role="tabpanel" aria-labelledby="tab-bay-2" className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {projects.map((p) => (
                <article key={p.id} className="p-3 bg-[#2B0F14] border border-[#D4A24E]/20 rounded-lg flex flex-col justify-between">
                  <div>
                    <h6 className="font-bold text-xs text-[#F5EBDD] mb-0.5 font-sans">{p.title}</h6>
                    <p className="text-[10px] text-[#D4A24E] mb-1.5">{Array.isArray(p.category) ? p.category.join(', ') : p.category}</p>
                    <p className="text-xs text-[#D8C6B6] leading-relaxed mb-2 line-clamp-3">{getLocalized(p.description)}</p>
                    {p.tech && (
                      <div className="flex flex-wrap gap-1 mb-2.5">
                        {p.tech.map((t, idx) => (
                          <span key={idx} className="text-[9px] bg-[#3C6E71]/25 border border-[#3C6E71]/50 text-[#F5EBDD] px-1.5 py-0.5 rounded">
                            {t}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="pt-2 border-t border-[#D4A24E]/20 flex gap-3 text-xs">
                    {p.project_url && (
                      <a href={p.project_url} target="_blank" rel="noopener noreferrer" className="font-bold text-[#D4A24E] hover:underline">
          ...
          <div id="panel-bay-3" role="tabpanel" aria-labelledby="tab-bay-3" className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {cvData.skills.map((s, idx) => (
                <div key={idx} className="p-3 bg-[#2B0F14] border border-[#D4A24E]/20 rounded-lg">
                  ...
                  <ul className="text-xs text-[#D8C6B6] space-y-0.5">
                    {s.items.map((it, i) => (
                      <li key={i} className="flex items-center gap-1.5">
                        <span className="w-1 h-1 bg-[#3C6E71] rounded-full shrink-0" aria-hidden="true"></span>
                        <span>{it}</span>
                      </li>
                    ))}
                  </ul>
